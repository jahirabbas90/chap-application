        ...
        _createClass(SingleUserChatPage, [{
          ...
          value: function ngOnInit() {
            if (this._currentChat != null) {
              console.warn(this._currentChat); // this._connecter.messageSeenBy(this._chatId, this._authUsr.id,this._visible_name);
            }
          }
        }, {
          key: "ngAfterViewInit",
                    ...
                    if (newUserArray[index].uid != this._userId) {
                      ...
                      if (newUserArray[index].fcm_token != undefined && newUserArray[index].fcm_token != null) {
                        this._connecter.sendFirebasePush(newUserArray[index].fcm_token).subscribe(function (result) {}, function (error) {
                          _this5._connecter.presentToast('error : ' + error); // this._connecter.showSuccess('error', error);
                    ...
                    if (newUserArray[index].fcm_token != undefined) {
                      if (newUserArray[index].uid != this._userId) {
                        ...
                        if (newUserArray[0].fcm_token != undefined) {
                          this._connecter.sendFirebasePush(newUserArray[0].fcm_token).subscribe(function (result) {}, function (error) {
                            _this5._connecter.presentToast('error : ' + error); // this._connecter.showSuccess('error', error);
